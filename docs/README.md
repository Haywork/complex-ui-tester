...
| 13 | [Claude Code workflows (the agentic-native primary surface)](./13-claude-code-workflows.md) | Engineers evaluating how to use CUIT day-to-day, prospects asking "how does my team use this" | The MCP server + `/cuit-instrument` + `/cuit-loop` as the primary surface. 12 wired tools. Five real conversation patterns (segment-drag regression, fresh Next.js instrumentation, flake investigation, PR gating, SOC 2 audit). When to use REST instead. The three positioning pillars (loop is durable, ease is the moat, Claude Code is primary). |
| 14 | [Agent Experience (AX)](./14-agent-experience.md) | Agent-tool integrators, MCP implementers, anyone evaluating loop quality | How the loop is designed for the model's constraints. The AX contract: outcome-first envelopes, token-budgeted payloads, agent-oriented errors, idempotent-by-default, resumable by ID, one-shot composition for autonomous workflows, self-measuring loop telemetry (turns-to-green, tokens-per-closed-loop). |
