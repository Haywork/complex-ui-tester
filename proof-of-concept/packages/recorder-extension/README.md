...
`SessionEvent[]` JSON that `@haywork/cuit-spec-gen` consumes. This is the missing
...
`@haywork/cuit-spec-gen` needs (pointer events with semantic `targetName`, plus
...
5. Paste into Claude Code / Codex with a prompt like:
   ...
   > Use `@haywork/cuit-spec-gen` to convert this session into a Playwright spec
   > against `@haywork/cuit-harness` primitives. Then run it. Make it pass.
...
`@haywork/cuit-types` exactly. Same shape `@haywork/cuit-spec-gen` already consumes.
...
- The recorder logic in `content.js` is duplicated from `@haywork/cuit-recorder`
