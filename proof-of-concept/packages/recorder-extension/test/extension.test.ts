 ...
 *   2. The generated content.js bundle (built from @cuit/recorder via build.mjs)
 ...
 *      satisfies the @cuit/types SessionEvent union. This is the artifact the popup
 *      copies/downloads and that @cuit/spec-gen consumes.
 *
 * Design notes:
 *  - Concern 3 drives the REAL Recorder from @cuit/recorder in Node using an
 ...
 *    @cuit/types union — never against the recorder's own output as its own oracle.
 ...
 *    7. Keyboard event capture: @cuit/types defines KeyboardEvent but the Recorder
...
// ─── Domain constants (mirror @cuit/types — the source of truth) ──────────────
...
describe('built content.js carries the full @cuit/recorder surface', () => {
  let bundleText = '';
 ...
 * @cuit/types union — deliberately NOT importing the recorder's own typing —
...
describe('a driven recorder exports a schema-valid SessionEvent[] session', () => {
  ...
  test('every event passes the independent @cuit/types schema validator', () => {
    const session = runSession();
...
//
// @cuit/types defines a `KeyboardEvent` union member (type:'keyboard') and
// exports `isKeyboardEvent`. The @cuit/recorder Recorder class is expected to
