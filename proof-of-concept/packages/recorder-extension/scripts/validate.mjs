...
// @haywork/cuit-recorder but forgot to update content.js" drift, plus a few
// manifest invariants Chrome will otherwise reject silently. Run via
// `pnpm -F @haywork/recorder-extension validate`.
...
// --- 4. content.js bundles @haywork/cuit-recorder primitives ---------------------
// content.js is generated by build.mjs (esbuild IIFE bundle of @haywork/cuit-recorder).
...
try {
  ...
  for (const a of REQUIRED_SEMANTIC_ATTRS) {
    check(recorderSrc.includes(a),
      `@haywork/cuit-recorder source must also list semantic attribute "${a}" — drift`);
  }
} catch (err) {
  failures.push(`could not read @haywork/cuit-recorder source for cross-check: ${err.message}`);
}
