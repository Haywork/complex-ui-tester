...
// @haywork/recorder but forgot to update content.js" drift, plus a few
...
// --- 4. content.js bundles @haywork/recorder primitives ---------------------
// content.js is generated by build.mjs (esbuild IIFE bundle of @haywork/recorder).
...
try {
  ...
  for (const a of REQUIRED_SEMANTIC_ATTRS) {
    check(recorderSrc.includes(a),
      `@haywork/recorder source must also list semantic attribute "${a}" — drift`);
  }
} catch (err) {
  failures.push(`could not read @haywork/recorder source for cross-check: ${err.message}`);
}
