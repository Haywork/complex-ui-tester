/**
 * ax-debug.ts — Pure-TS debugging primitives for @cuit/runner.
 *
 ...
 *   - setClock, dispatchDrag: call into @cuit/harness (executable)
...
export function listPrimitives(): PrimitiveEntry[] {
  ...
  const entries: PrimitiveEntry[] = [
    {
      kind: 'setClock',
      mode: 'executable',
      realizedBy: '@cuit/harness setClock via runPrimitive',
    },
    {
      kind: 'dispatchDrag',
      mode: 'executable',
      realizedBy: '@cuit/harness dispatchDrag via runPrimitive',
    },
    {
      kind: 'goto',
      mode: 'noop',
  ...
  // If a new Primitive kind is added to @cuit/types but not above,
